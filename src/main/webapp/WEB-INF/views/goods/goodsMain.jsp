	...
	$("#rentBtn").on("click", function(){
		...
		if(parseInt($('#demo_vertical').val()) <= parseInt(gdsCnt)){
			swalWithBootstrapButtons({
				...
				if (result.value) {
					swalWithBootstrapButtons('대여하셨습니다.', '', 'success').then(function(result){
			...
			swalWithBootstrapButtons('수량이 부족합니다.', '', 'error').then(function(result){
		            ...
		            <table class="table table-bordered table-hover mb-4">
		                <thead>
		                    <tr>
		                        ...
		                        <th>비품명</th>
		                        <th style="width: 150px;">잔여 물량</th>
		                        <th class="text-center" style="width: 180px;">비품상태</th>
		                        <th style="width: 180px;">대여하기</th>
		                    </tr>
		                </thead>
		                <tbody>
			                <c:forEach var="GdsMngVO" items="${list.content}">
			                    <tr>
			                        ...
			                        <c:choose>
			                        	<c:when test="${GdsMngVO.gdsStock - GdsMngVO.rntCnt == 0}">
				                        	<td class="text-center" style="padding: 7px;"><button class="btn btn-outline-primary modalClick" data-toggle="modal" data-target="#rentModalCenter" disabled>대여불가</button></td>
		                        		</c:when>
		                        		<c:otherwise>
		                        			<td class="text-center" style="padding: 7px;"><button class="btn btn-outline-primary modalClick" data-toggle="modal" data-target="#rentModalCenter">대여</button></td>
		                        		</c:otherwise>
			                        </c:choose>
			                    </tr>
			                </c:forEach>
