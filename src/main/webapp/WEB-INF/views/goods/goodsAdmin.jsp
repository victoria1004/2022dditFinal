...
// 		var rntNo = $(this).closest("tr").children().first().val();
		...
		swalWithBootstrapButtons({
			...
			if (result.value) {
				swalWithBootstrapButtons('반납하셨습니다.', '', 'success').then(function(result){
					if(result.value){
						$.ajax({
							url: "/gds/rtnUser",
							data: {"rntNo": rntNo},
	...
	$("#rentBtn").on("click", function(){
		...
		swalWithBootstrapButtons({
			...
			if (result.value) {
				swalWithBootstrapButtons('등록하셨습니다.', '', 'success').then(function(result){
			...
			// Read more about handling dismissals
			result.dismiss === swal.DismissReason.cancel) {
				swalWithBootstrapButtons('등록이 취소되었습니다', '', 'error')
			}
		})
	})
	...
	$(document).on("click", ".deleteGoods", function(e){
		...
		swalWithBootstrapButtons({
			...
			if (result.value) {
				swalWithBootstrapButtons('삭제하셨습니다.', '', 'success').then(function(result){
					$.ajax({
						url: "/gds/goodsDelete",
						data: {"gdsNo": gdsNo},
						type : "post",
		                ...
		                <tbody>
			                <c:forEach var="GdsMngVO" items="${list.content}">
			                    <tr>
                        			...
                        			<td class="text-center" style="padding: 7px;"><button class="btn btn-outline-primary modalClick" data-toggle="modal" data-target="#rentModalCenter">조회</button></td>
