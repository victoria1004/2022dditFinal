...
.on('drag', function (el) {
  ...
  //console.log("왔냐>>>",el); 
  //var findit = el.childNodes[1].firstChild.firstChild.firstChild.innerText;
	//console.log(el.firstChild.firstChild.firstChild.value);
	var findit = el.firstChild.firstChild.firstChild.value;
  var parentNm = el.parentNode;
 //console.log(">>>", parentNm);
  if(parentNm.getAttribute("id") == "right-defaults"){
  	//console.log("!!!");
	var mbrCtgrCd = "M02"; // m03 -> m02로 사용자 승인되는 경우 변수 하나 생성해서 파라미터로 넘겨준다.
	updateUsers(findit, mbrCtgrCd);
  } else {
	var mbrCtgrCd = "M03"; // m02 -> m03으로 사용자 미승인으로 변경되는 경우의 변수
	updateUsers(findit, mbrCtgrCd);
	}
