 ...
 * @author 이영지(슈퍼개발자K3)
 */
package kr.or.ddit.chat.server.model.encoder;
...
public class MessageEncoder implements Encoder.Text<Message> {
	...
	public String encode(Message message) throws EncodeException {
		...
		if (message instanceof ChatMessage) {
			...
			result = Json.createObjectBuilder().add("messageType", chatMessage.getClass().getSimpleName()).add("name", chatMessage.getName()).add("message", chatMessage.getMessage()).build().toString();
	...
	private String buildJsonUserData(Set<String> set, String messageType) {
		...
		return Json.createObjectBuilder().add("messageType", messageType).add("users", jsonArrayBuilder).build().toString();
	}
