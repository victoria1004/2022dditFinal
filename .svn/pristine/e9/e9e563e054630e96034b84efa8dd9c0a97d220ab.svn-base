...
<script>
	...
	var pjtNm = "<c:out value='${project.pjtNm}' />"; //세션에 세팅되면 해당 프로젝트 번호를 저장시킨다.
	var mbrId = "<c:out value='${member.mbrId}' />";
	var start = 1; //모든 파일 가져올 때 시작 rownum값
	var end = start + 19; //모든 파일 가져올 때 끝 rownum값 - 20개씩 나뉘어 출력되게 한다.
	...
	function dirOpen(el) {
		...
		getLeaf(pjtNo, folderNm); //클릭한 메뉴 폴더에 있는 하위 폴더 목록을 가져온다.
		getfldrFiles(pjtNo, folderNm); //클릭한 메뉴 폴더에 있는 모든 파일들 가져온다.

	}

	$(function() {

		getAllFiles(pjtNo, start, end); //메인 접속 시 바로 ajax로 모든 파일 목록을 출력한다.

		//메인 접속 시 프로젝트이름으로 최상단 폴더를  좌측 메뉴에 출력한다.
		getParentFolder(pjtNm);

		// 파일 업로드 버튼 클릭 시 해당 프로젝트가 가지고 있는 폴더이름 뿌리기
		...
		//파일 검색 시 실시간으로 검색한 파일명과 관련된 파일들 뿌리기
				...
				<div class="search-input-group-style input-group mb-3 searchDiv">
		         ...
		         <line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>
					...
					<input style="height: 45px;" type="text" id="searchName"
						class="form-control" placeholder="파일을 검색하세요" aria-label="Username"
									...
									<div class="input-group mb-3">
										<div class="disFlex">
											...
											<input type="hidden" id="n-title" name="leafFolder"
												class="form-control" maxlength="20" placeholder="폴더명을 입력하세요">
											<div id="fileDiv" class="custom-file" style="display: none;">
												<input type="file" class="custom-file-input" name="files"
													...
													id="fileLabel" class="custom-file-label" for="files">파일을
													업로드하세요</label>
											</div>
										</div>
									</div>
