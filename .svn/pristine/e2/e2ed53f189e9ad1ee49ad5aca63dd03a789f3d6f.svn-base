...
function leafAdd(){
	...
	$('input:radio').each(function(){
		$(this).prop("checked", false); //모달창 꺼지면서 체크된 애들도 해제
	})
	getLeaf(leaf, pjtNo);
	...
	<div class="">
		<div id="allCont" class="widget-content-area br-4">
 			<div class="btns" style="z-index:1; position: relative; top: 710px;">
				<button class="btn btn-outline-primary mb-2" onclick="location.href='/project/mkTodo.do?pjtNo=${map.PJT_NO}&tdNo=${map.TD_NO}'">편집</button>
				<c:if test="${map.TD_WRTR_ID eq mbrNm}">
					<button class="btn btn-outline-primary mb-2" onclick="deleteTodo('${map.TD_NO}')">삭제</button>
				</c:if>
			</div>
			<div id="sepeCont">
			<div class="todoContent"
				...
				<div class="widget-one">
					<label class="p_lab" for="tdCtgrCd">유형</label> &nbsp;&nbsp; 
				...
				<br>
				<div class="widget-one">
					<label class="p_lab" for="tdTtl">일감 제목</label>
				...
				<br>
				<div class="widget-one">
					<label class="p_lab" for="tdCn">업무 내용</label>
					&nbsp;&nbsp;
				...
				<br>
				<div class="widget-one">
					<label class="p_lab" for="tdDate">업무 기간</label> &nbsp;&nbsp;
				...
				<div class="widget-one"style="position: relative; left: -18px; width: 580px;">
					<label class="p_lab" for="tdWrtrId">지정자</label> &nbsp;&nbsp;
					 <div class="form-control" style="width: 20%; position: relative; left:-12px;" >${map.TD_WRTR_ID}</div>
					<label class="p_lab" for="tdPicId" style="position: relative; left: -40px;" >담당자</label> &nbsp;&nbsp;
				...
				<div class="widget-one"
					...
					<label class="p_lab" for="tdImprCd">중요도</label> &nbsp;&nbsp; 
					<div class="form-control" style="width: 20%; position: relative; left:-12px;">
					${map.TD_IMPR_CD}
					</div>	
					<label class="p_lab" style="position: relative; left: -40px;" for="tdStts">진행상태</label>
					&nbsp;&nbsp; 
				...
				<br>
				<div class="widget-one">
					<label class="p_lab" for="prgrs">진척도</label> &nbsp;&nbsp;
			 ...
			 <div class="todoContent" style="width: 48%; padding: 10px; margin-top: 10px; position: relative; right: -30px;">
				<div class="widget-one">
				<label class="p_lab" id="leaf" style="position: relative; top: 7px; right: 20px;">관련 일감</label> &nbsp;&nbsp; 
				 ...
				 <c:if test="${leaf != null}">변경</c:if>
				 <c:if test="${leaf eq null}">추가</c:if></button>
				</div>
				...
				<br />
				<div class="widget-one">
				<label class="p_lab" id="leaf" style="position: relative; right: 38px; top: -30px;">이력</label> &nbsp;&nbsp; 
				</div>
				<div class="widget-content ps__thumb-y" style="width: 460px;">
					<c:forEach items="${histories}" var="his">
						...
						<ul class="historyList">
							<c:choose>
								<c:when test="${his.COLUMN_NM eq '유형' || his.COLUMN_NM eq '중요도'||his.COLUMN_NM eq '진행상태'}">
									<li class="history" style="font-size:15px;">${his.PRTPNT_ID}님이
									 <span class="bs-tooltip" title="${fn:substring(his.CHG_DT,0,19)}">
									 ${fn:substring(his.CHG_DT,5,7)}월  ${fn:substring(his.CHG_DT,8,10)}일에</span> ${his.COLUMN_NM}을(를) <br>
									  <span class="textZone">'${his.BEFORE_NM}'</span>에서 <span class="textZone">'${his.AFTER_NM}'</span>(으)로 변경하였습니다.</li>						
								</c:when>
								<c:when test="${his.COLUMN_NM eq '담당자'}">
									<li class="history" style="font-size:15px;">${his.PRTPNT_ID}님이 
									 <span class="bs-tooltip" title="${fn:substring(his.CHG_DT,0,19)}">
									 ${fn:substring(his.CHG_DT,5,7)}월  ${fn:substring(his.CHG_DT,8,10)}일에</span> ${his.COLUMN_NM}을(를) <br>
									 <span class="textZone">'${his.BEFORE_MEM}'</span>에서 <span class="textZone">'${his.AFTER_MEM}'</span>(으)로 변경하였습니다.</li>						
								</c:when>
								<c:otherwise>
									<li class="history" style="font-size:15px;">${his.PRTPNT_ID}님이
									 <span class="bs-tooltip" title="${fn:substring(his.CHG_DT,0,19)}">
									 ${fn:substring(his.CHG_DT,5,7)}월  ${fn:substring(his.CHG_DT,8,10)}일에</span> ${his.COLUMN_NM}을(를) <br>
									  <span class="textZone">'${his.BEFORE_CN}'</span>에서 <span class="textZone">'${his.AFTER_CN}'</span>(으)로 변경하였습니다.</li>						
								</c:otherwise>
							</c:choose>
						</ul>
						</div>
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" id="myExtraLargeModalLabel">일감 목록</h5>
                    ...
                    <table class="table" style="width:100%;">
                        <thead>
                            <tr>
                                ...
                                <th><div class="th-content">일감번호</div></th>
                                <th style="width:400px;"><div class="th-content">일감</div></th>
                                <th><div class="th-content">담당자</div></th>
                                <th></th>
                            </tr>
                        </thead>
                        <tbody>
        ...
        <div class="modal-footer">
            ...
            <button type="button" class="btn btn-primary" onclick="leafAdd()">완료</button>
        </div>
    </div>
  </div>
</div>
